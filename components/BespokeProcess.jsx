...
// ── Process Steps (visual-only skeleton; text lives in messages.bespoke.steps) ─
const PROCESS_STEPS = [
  {
    number: '01',
    icon: (
    ...
    >
          ...
          >
            ...
            >
              {trustPoints.map((text) => (
                <motion.li key={text} className={styles.trustItem} variants={fadeUp}>
      ...
      <div className={styles.processBlock}>
        <div className="container">
          ...
          >
            <span className="micro-label">{t('processMicroLabel')}</span>
          ...
          >
            {PROCESS_STEPS.map((step, i) => (
              <motion.div key={step.number} className={styles.stepCard} variants={fadeUp}>
                ...
                <h4 className={styles.stepTitle}>{steps[i].title}</h4>
                <p className={styles.stepDesc}>{steps[i].desc}</p>
                <div className={styles.stepTime}>{t('estTime', { timeframe: steps[i].timeframe })}</div>
